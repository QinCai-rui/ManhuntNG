...
public class HunterSubcommand implements Subcommand {
    ...
    public boolean execute(CommandSender sender, ManhuntNG plugin, String[] args) {
        if (args.length < 2) {
            sender.sendMessage(cfg(plugin).getMessageComponent("usage.hunter"));
            return true;
        }
        ...
        sender.sendMessage(cfg(plugin).getMessageComponent("role.set-hunter-sender", "{player}", target.getName()));
        target.sendMessage(cfg(plugin).getMessageComponent("role.set-hunter-target"));
        return true;
    }
}
